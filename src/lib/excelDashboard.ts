...
export async function dashboardDataFromExcel(file: File): Promise<DashboardData> {
  const raw = (await readXlsxFile(file, { trim: true })) as unknown;
  const sheets = normalizeSheets(raw);
  ...
  return createDashboardData(file.name.normalize("NFC"), sheet.sheet, rows, sheets);
}

function normalizeSheets(raw: unknown): WorkbookSheet[] {
  if (
    Array.isArray(raw) &&
    raw.length > 0 &&
    typeof raw[0] === "object" &&
...
function parseRows(
  ...
  return rows.flatMap((row) => {
    const date = parseDate(row[columns.date] as Cell);
    const amount = parseAmount(row[columns.amount] as Cell);
    const department = stringify(row[columns.department] as Cell) || "미지정";

    if (!date || !amount || amount <= 0 || !department) {
      return [];
    }

    const item = columns.item >= 0 ? stringify(row[columns.item] as Cell) : "";
    const vendor = columns.vendor >= 0 ? stringify(row[columns.vendor] as Cell) : "";
    const rawCategory = columns.category >= 0 ? stringify(row[columns.category] as Cell) : "";
  ...
  for (const sheet of sheets) {
    for (const row of sheet.data) {
      for (const cell of row) {
        const text = stringify(cell as Cell);
...
function extractPriorYearTotal(sheets: WorkbookSheet[]) {
  for (const sheet of sheets) {
    ...
    for (const row of sheet.data) {
      if (row.some((cell) => stringify(cell as Cell).includes("2025년"))) {
        inPriorYearBlock = true;
      }

      if (inPriorYearBlock && row.some((cell) => stringify(cell as Cell) === "합계")) {
        const amounts = row.map((cell) => parseAmount(cell as Cell)).filter((value) => value > 0);
        if (amounts.length > 0) {
