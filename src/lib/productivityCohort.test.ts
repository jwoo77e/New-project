...
describe("buildProductivityExecutiveModel", () => {
  ...
  it("aligns observable monthly usage with the same cost month", () => {
    ...
    expect(model.activeUsers).toBe(33);
    expect(model.licensedUsers).toBe(33);
  ...
  it("builds separate adoption, activity, and output KPI stages", () => {
    ...
    expect(model.axKpis.adoption.evidenceCoverageRate).toBeCloseTo((3 / 33) * 100, 5);
  ...
  it("reconciles active Claude seats with the latest spend activity", () => {
    expect(initialClaudeTeamUsageData.source.verification.memberAccounts).toBe(33);
    expect(initialClaudeTeamUsageData.source.verification.activeMemberAccounts).toBe(33);
    expect(initialClaudeTeamUsageData.source.verification.approvedAccounts).toBe(33);
    expect(initialClaudeTeamUsageData.source.verification.approvedButNoUsage).toBe(1);
    expect(initialClaudeTeamUsageData.users).toHaveLength(33);
    expect(initialClaudeTeamUsageData.activeUsers).toBe(33);
    expect(initialClaudeTeamUsageData.spendUsers).toBe(31);
